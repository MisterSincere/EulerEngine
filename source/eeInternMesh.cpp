...
namespace vk
{
  namespace intern
  {
    ...
    Mesh::Mesh(vk::VulkanRenderer* renderer)
      : renderer(renderer)
    {
      assert(renderer);
    }
    ...
    void Mesh::Create(const char* file)
    {
      if (isCreated)
      {
    ...
    void Mesh::Create(const std::vector<EEVertex>& vertices, const std::vector<uint32_t>& indices)
    {
      if (isCreated)
      {
        EEPRINT("Mesh already created. Mesh creation aborted!\n");
    ...
    void Mesh::Create(void* data, size_t bufferSize, std::vector<uint32_t>& indices)
    {
      if (isCreated)
      {
        EEPRINT("Mesh already created. Mesh creation aborted!\n");
    ...
    void Mesh::Record(VkCommandBuffer cmdBuffer)
    {
      // Bind buffer
